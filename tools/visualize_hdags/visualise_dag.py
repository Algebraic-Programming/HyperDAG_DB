...
def _font_weight_sink_nodes(
    G: nx.DiGraph, sink_nodes: list, not_sink="normal", sink="bold"
) -> List[str]:
    # Weight sink nodes
...
def _node_color_sink_nodes(
    G: nx.DiGraph, sink_nodes: list, not_sink="k", sink="r"
) -> List[str]:
    # Colorise sink nodes
...
def _lerp(minimum, maximum, value) -> Any:
...
def _get_weights(G: nx.DiGraph) -> Tuple[float, float, List[float]]:
    ...
    return (min(weights), max(weights), weights)
...
def _arrowsize_by_weight(G: nx.DiGraph, min_size=10, max_size=20) -> List[float]:
    ...
    return [
        _lerp(min_size, max_size, (w - min_w) / (max_w - min_w)) for w in edges_weights
    ]
...
def _edgewidth_by_weight(G: nx.DiGraph, min_width=0.5, max_width=3) -> list:
    ...
    return [
        _lerp(min_width, max_width, (w - min_w) / (max_w - min_w))
...
def command(filename) -> None:
    ...
    print(f"Searching for sink nodes ...")
    sink_nodes = _find_sink_nodes(G)
    print(f"Generating the graph rendering ...")
    nx.draw(
        G,
    ...
    plt.savefig(
        filename + ".png", dpi=312, format="png", bbox_inches="tight", transparent=True
    )
